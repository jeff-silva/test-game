import { FirstPersonControls, ExtendedObject3D } from "enable3d";
...
export default class extends Script {
  ...
  onCreate(scope) {
    ...
    scope.game.scene.add(this.player);

    this.firstPersonControl = new FirstPersonControls(
      scope.game.camera,
      this.player,
      {
        //
    ...
    this.playerMove = this.inputDefine({
      front: () => {
        if (this.keyboard.w) return 1;
        if (this.keyboard.s) return -1;
        return 0;
      },
      side: () => {
        if (this.keyboard.a) return -1;
        if (this.keyboard.d) return 1;
        return 0;
      },
    });

    this.on("update", (scope) => {
      console.log(this.playerMove.front);
    });

    // scope.on("update", (scope) => {
    //   this.locked = document.pointerLockElement == scope.canvas.target;

    //   // Object move
    //   (() => {
    //     if (this.input.front == 0) return;
    //     console.log("go");
    //     const THREE = scope.THREE;
    //     const object = this.player;
    //     const distance = this.input.front / 2;
    ...
    // scope.on("input", (scope) => {
    //   if (scope.event.type == "click") {
    //     if (!this.locked) {
    //       scope.canvas.target.requestPointerLock();
    //     }
    //   }
    //   if (scope.event.type == "pointermove") {
    //     if (this.locked) {
    //       this.firstPersonControl.update(
    //         scope.event.movementX,
    //         scope.event.movementY
    //       );
    //     }
    //   }
    // });
